...
abstract class BaseSetupDialogFragment extends DialogFragment{
	...
	protected def createDialog(dialogView: View, messageID: Int, positiveButtonString: Int, onClickFunction: =>  Unit): Dialog = {
    ...
    builder.setMessage(getString(messageID))
      ...
      .setPositiveButton(positiveButtonString, new OnClickListener {
        def onClick(dialog: DialogInterface, which: Int) {
          onClickFunction
          }
      })
